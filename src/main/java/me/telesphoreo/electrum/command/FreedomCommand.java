...
public abstract class FreedomCommand implements CommandExecutor
{
    ...
    public void register()
    {
        ...
        {
            unregisterNukkitCommand((PluginCommand) Electrum.plugin.getServer().getPluginCommand(cmd.getName()));
    ...
    private void unregisterNukkitCommand(PluginCommand command)
    {
        try
        {
            Object result = getPrivateField(Electrum.plugin.getServer().getPluginManager(), "commandMap");
        ...
        {
            Electrum.plugin.getLogger().alert("", ex);
        }
    }
